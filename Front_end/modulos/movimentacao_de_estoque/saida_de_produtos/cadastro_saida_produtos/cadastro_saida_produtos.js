...
export default async function cadastro_saida_produtos(dados) {
    select2("100%")
    dataAtual()
    ...
    formsaidaProduto.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:3000/saida_produto', {
                method: 'POST',
                headers: {
    ...
    document.querySelector("#btn_voltar_saida_produtos").addEventListener("click", async () => {
        let formData = new FormData(formsaidaProduto);
        let data = Object.fromEntries(formData);
